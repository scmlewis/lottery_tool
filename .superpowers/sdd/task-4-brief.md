...
**Interfaces:**
- Consumes: Types, state management, hooks
- Produces: Main application shell
...
@layer base {
  body {
    @apply bg-background text-on-surface font-sans select-none;
  }
}
...
.confetti-piece {
  ...
  top: -10px;
  border-radius: 2px;
  animation: confetti-fall var(--confetti-duration) var(--confetti-delay) linear forwards;
...
git commit -m "feat: add main App component with navigation and settings"
```